...
// Default values for HeaderAura
const AURA_DEFAULTS: HeaderData = {
  showSearch: true,
  showAccount: true,
  showCart: true,
...
// 12. HeaderAura - "Premium Glow" (Gradient accents with ambient lighting)
export const HeaderAura: React.FC<HeaderProps> = ({
  storeName,
  logoUrl,
  logoHeight,
  links,
  ...
  const settings = { ...AURA_DEFAULTS, ...data };
  const [searchOpen, setSearchOpen] = React.useState(false);
          ...
          >
            ...
            {logoUrl ? (
              ...
              <span 
                ...
                style={{ 
                  backgroundImage: `linear-gradient(135deg, ${settings.textColor}, ${settings.accentColor})`,
                }}
              >
                {storeName}
              </span>
          ...
          <nav className="hidden md:flex items-center gap-1 flex-1 justify-center">
            {(links || []).map(l => (
              <a
                ...
                className="px-4 py-2 text-sm font-medium tracking-wide relative group transition-all"
                style={{ color: l.active ? settings.accentColor : settings.textColor }}
              >
                {l.label}
                <span 
                  className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 transition-all duration-300 ${
                    l.active ? 'w-full' : 'w-0 group-hover:w-full'
                  }`}
                  style={{ backgroundColor: settings.accentColor }}
                />
              </a>
          ...
          <div className="flex items-center gap-2 shrink-0">
                ...
                {searchOpen ? (
                  <div className="relative">
                    <input 
                      type="text" 
                      placeholder={settings.searchPlaceholder}
                      className="w-56 px-4 py-2 pr-10 text-sm rounded-full focus:outline-none transition-all"
                      style={{ 
            ...
            {settings.showCart && (
              ...
              >
                ...
                {cartCount > 0 && (
                  <span 
                    className="absolute -top-1 -right-1 w-5 h-5 rounded-full text-xs flex items-center justify-center font-bold ring-2"
                    style={{ 
                      backgroundColor: settings.cartBadgeColor,
                      color: settings.cartBadgeTextColor,
                      ringColor: settings.backgroundColor,
                    }}
                  >
                    {cartCount}
                  </span>
...
// Default values for HeaderQuantum
const QUANTUM_DEFAULTS: HeaderData = {
  showSearch: true,
  showAccount: true,
  showCart: true,
  searchPlaceholder: "Search...",
...
// 13. HeaderQuantum - "Futuristic Grid" (Geometric patterns with animated hover states)
export const HeaderQuantum: React.FC<HeaderProps> = ({
  ...
  const settings = { ...QUANTUM_DEFAULTS, ...data };
  const [searchOpen, setSearchOpen] = React.useState(false);
        ...
        <div className="flex items-center justify-between gap-8">
          ...
          >
            ...
            >
              <Hexagon 
                size={20} 
                className="absolute group-hover:rotate-180 transition-transform duration-700"
          ...
          {/* Navigation with geometric separators */}
            ...
            {(links || []).map((l, idx) => (
              <React.Fragment key={l.href}>
                <a
                  ...
                  className="px-5 py-2.5 text-sm font-bold uppercase tracking-wider relative group"
                  style={{ color: l.active ? settings.accentColor : settings.textColor }}
                >
                  {l.label}
                  {/* Animated underline */}
                  <span 
                    className={`absolute bottom-0 left-0 right-0 h-0.5 transform origin-left transition-transform duration-300 ${
                      l.active ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'
                    }`}
                    style={{ backgroundColor: settings.accentColor }}
                  />
                  {/* Corner accents */}
                  <span 
                    className="absolute top-0 left-0 w-1.5 h-1.5 opacity-0 group-hover:opacity-100 transition-opacity"
                    style={{ 
                  ...
                  />
                  <span 
                    className="absolute bottom-0 right-0 w-1.5 h-1.5 opacity-0 group-hover:opacity-100 transition-opacity"
                    style={{ 
                      borderBottom: `2px solid ${settings.accentColor}`,
                      borderRight: `2px solid ${settings.accentColor}`,
                    }}
                  />
                </a>
                {idx < (links || []).length - 1 && (
          ...
          {/* Actions with geometric styling */}
          <div className="flex items-center gap-1 shrink-0">
                ...
                {searchOpen ? (
                  <div className="relative">
                    <input 
                      type="text" 
                      placeholder={settings.searchPlaceholder}
                      className="w-52 px-4 py-2 pr-10 text-sm focus:outline-none transition-all"
                      style={{ 
                        backgroundColor: settings.searchFocusBackgroundColor,
                        border: `2px solid ${settings.searchFocusBorderColor}`,
                        color: settings.searchInputTextColor,
                  ...
                  <button
                    onClick={() => setSearchOpen(true)}
                    className="p-2.5 relative group overflow-hidden"
                    style={{ 
                      border: `1px solid ${settings.borderColor}`,
                      color: settings.textColor,
                    }}
            ...
            {settings.showCart && (
              <button 
                onClick={onOpenCart}
                className="relative p-2.5 group overflow-hidden"
                style={{ 
                  border: `2px solid ${settings.accentColor}`,
                  color: settings.accentColor,
                }}
              >
                ...
                {cartCount > 0 && (
                  <span 
                    className="absolute -top-2 -right-2 w-5 h-5 text-[10px] flex items-center justify-center font-bold"
                    style={{ 
                      backgroundColor: settings.cartBadgeColor,
                      color: settings.cartBadgeTextColor,
                      clipPath: 'polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)',
