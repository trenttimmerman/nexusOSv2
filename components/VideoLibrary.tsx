...
export const VIDEO_COMPONENTS: Record<string, React.FC<any>> = {
  'vid-full': ({ data, isEditable, onUpdate }) => (
    <div className="relative w-full aspect-video md:h-[80vh] overflow-hidden group" style={{ backgroundColor: data?.backgroundColor || '#000000' }}>
      <video 
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="w-20 h-20 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform" style={{ backgroundColor: (data?.playButtonColor || '#ffffff') + '33', backdropFilter: 'blur(10px)' }}>
          <Play size={32} fill={data?.playButtonColor || '#ffffff'} style={{ color: data?.playButtonColor || '#ffffff' }} className="ml-1" />
        </div>
      </div>
    </div>
  ),
  ...
  'vid-window': ({ data, isEditable, onUpdate }) => (
    <div className="py-20 px-6 max-w-6xl mx-auto" style={{ backgroundColor: data?.backgroundColor || 'transparent' }}>
      ...
      <div className="aspect-video rounded-2xl overflow-hidden shadow-2xl relative group" style={{ backgroundColor: data?.backgroundColor || '#000000' }}>
        <video 
          src={data?.videoUrl || DEFAULT_VIDEO} 
          className="w-full h-full object-cover"
          controls={data?.controls !== false}
        ...
        />
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="w-16 h-16 rounded-full flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform" style={{ backgroundColor: data?.playButtonColor || '#ffffff' }}>
            <Play size={24} fill={data?.backgroundColor || '#000000'} style={{ color: data?.backgroundColor || '#000000' }} className="ml-1" />
          </div>
        </div>
      </div>
    </div>
  ...
  'vid-bg': ({ data, isEditable, onUpdate }) => {
    const overlayOpacity = (data?.overlayOpacity || 40) / 100;
    return (
      <div className="relative h-[600px] overflow-hidden flex items-center justify-center text-center" style={{ backgroundColor: data?.backgroundColor || '#000000' }}>
        <video 
          src={data?.videoUrl || DEFAULT_VIDEO} 
        ...
        <div className="relative z-10 max-w-2xl px-6">
          ...
          />
          {data?.buttonText && (
            <a href={data?.buttonLink || '#'} className="inline-block mt-8 px-8 py-3 rounded-full font-bold transition-colors" style={{ backgroundColor: data?.buttonBackground || '#ffffff', color: data?.buttonTextColor || '#000000' }}>
              {data.buttonText}
            </a>
          )}
        </div>
  ...
  'vid-split': ({ data, isEditable, onUpdate }) => (
    <div className="py-20 px-6 max-w-7xl mx-auto" style={{ backgroundColor: data?.backgroundColor || 'transparent' }}>
      <div className="grid md:grid-cols-2 gap-12 items-center">
        <div className="order-2 md:order-1">
          <div className="aspect-square rounded-2xl overflow-hidden relative group" style={{ backgroundColor: '#000000' }}>
            <video 
              src={data?.videoUrl || DEFAULT_VIDEO} 
              className="w-full h-full object-cover"
              autoPlay={data?.autoplay !== false}
        ...
        <div className="order-1 md:order-2">
          <EditableText
            ...
            style={{ color: data?.headingColor || '#000000' }}
          />
          <p className="text-lg mb-8 leading-relaxed" style={{ color: data?.textColor || '#6b7280' }}>
            {data?.subheading || 'Take a look at how we craft our products with care and precision. From the initial sketch to the final stitch, every step is a journey of passion.'}
          </p>
          {data?.buttonText && (
            <a href={data?.buttonLink || '#'} className="px-8 py-3 border-2 rounded-full font-bold inline-block transition-colors" style={{ borderColor: data?.buttonBackground || '#000000', color: data?.buttonBackground || '#000000' }}>
              {data.buttonText}
            </a>
          )}
        </div>
  ...
  'vid-popup': ({ data, isEditable, onUpdate }) => (
    <div className="py-24 px-6 max-w-7xl mx-auto text-center" style={{ backgroundColor: data?.backgroundColor || 'transparent' }}>
      <div className="relative max-w-4xl mx-auto aspect-video rounded-2xl overflow-hidden group cursor-pointer shadow-2xl" style={{ backgroundColor: '#171717' }}>
        <img 
          src={data?.thumbnailUrl || 'https://images.unsplash.com/photo-1492633423870-43d1cd2775eb?w=1200&q=80'} 
          alt="Video Thumbnail" 
          className="w-full h-full object-cover opacity-60 group-hover:opacity-40 transition-opacity"
        />
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <div className="w-24 h-24 rounded-full flex items-center justify-center mb-8 group-hover:scale-110 transition-transform duration-300" style={{ backgroundColor: data?.playButtonColor || '#ffffff' }}>
            <Play size={32} fill={data?.backgroundColor || '#000000'} style={{ color: data?.backgroundColor || '#000000' }} className="ml-2" />
          </div>
          <EditableText
  ...
  'vid-slider': ({ data, isEditable, onUpdate }) => (
    <div className="py-20 text-white overflow-hidden" style={{ backgroundColor: data?.backgroundColor || '#000000' }}>
      <div className="px-6 max-w-7xl mx-auto mb-12 flex justify-between items-end">
        ...
        <div className="flex gap-2">
          <button className="w-10 h-10 rounded-full border flex items-center justify-center hover:bg-white hover:text-black transition-colors" style={{ borderColor: (data?.headingColor || '#ffffff') + '33' }}><ChevronLeft size={20} /></button>
          <button className="w-10 h-10 rounded-full border flex items-center justify-center hover:bg-white hover:text-black transition-colors" style={{ borderColor: (data?.headingColor || '#ffffff') + '33' }}><ChevronRight size={20} /></button>
        </div>
      </div>
      ...
      <div className="relative h-screen w-full overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
          ...
          <div className="flex gap-4">
            {data?.buttonText && (
              <a href={data?.buttonLink || '#'} className="px-8 py-4 font-bold rounded-full transition-colors" style={{ backgroundColor: data?.buttonBackground || '#ffffff', color: data?.buttonTextColor || '#000000' }}>
                {data.buttonText}
              </a>
            )}
            <button className="px-8 py-4 border font-bold rounded-full transition-colors flex items-center gap-2" style={{ borderColor: data?.headingColor || '#ffffff', color: data?.headingColor || '#ffffff' }}>
              <Play size={16} fill={data?.headingColor || '#ffffff'} /> Watch Film
            </button>
          </div>
        </div>
      </div>
  ...
  'vid-interactive': ({ data, isEditable, onUpdate }) => (
    <div className="py-20 px-6 max-w-7xl mx-auto" style={{ backgroundColor: data?.backgroundColor || 'transparent' }}>
      <div className="relative aspect-video rounded-2xl overflow-hidden shadow-2xl" style={{ backgroundColor: '#000000' }}>
        <video 
          src={data?.videoUrl || DEFAULT_VIDEO} 
          className="w-full h-full object-cover"
          autoPlay={data?.autoplay !== false}
