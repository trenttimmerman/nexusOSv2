...
| `showCommandPalette` | `false` | Command palette (⌘K) |
...
[
  {
    ...
    "style": {
      // ALL fields from the tables above go here
      // Only include fields you're customizing (defaults handle the rest)
    },
    "data": {
      "logo": "BRAND_NAME",
...
6. **Nav Active Styles** - Choose different `navActiveStyle` for each variant to maximize visual contrast. Popular options: `dot`, `underline`, `capsule`, `glow`, `bracket`, `highlight`.
